...
| Calibrated app Portra 400, blue sky, 100% / 150% | 0.448 / 0.453 | 3.19 / 4.91 |
| Calibrated app Portra 800, blue sky, 100% / 150% | 0.438 / 0.442 | 4.24 / 6.34 |

The old app values sat below even the soft V800 reference and contradicted Kodak's visible-grain
index. The new 100% masters land inside the cross-scanner reference envelope; 150% is an
intentional stronger interpretation for the user control.

## 4. Model decision

- Raise density amplitude from `0.028 → 0.058` for Portra 400 and `0.040 → 0.088` for
  Portra 800.
- Keep the existing crystal sizes (`1.65`, `1.95`) and clumping (`0.13`, `0.22`). Their measured
  spatial correlation and non-Gaussian tails were already plausible. Making blobs larger would
  recreate the artificial specks/clouds seen in the retired grain implementation.
...
- Continue applying grain after lens blur, colour, skin/foliage/sky handling, and halation. The focused
  portrait render confirms that the grain stays sharp over an optically defocused background.

`DevelopPipelineTest.portraGrainIsVisibleAndKeepsTheMeasuredStockSeparationAtExportSize` guards
the 3000-pixel baseline amplitude and the Portra 400/800 separation. Existing tests separately
guard zero mean, highlight roll-off, deterministic non-repetition, no low-frequency clouds,
correlated colour, and heavier clumped tails.
